...
function initEditor() {
  ...
  const existingMap = localStorage.getItem('custom_map_data_v0.110');
  if (existingMap) {
    try {
...
function playTestMap() {
  const mapData = serializeMapData();
  localStorage.setItem('custom_map_data_v0.110', JSON.stringify(mapData));
...
function resetMap() {
  localStorage.removeItem('custom_map_data_v0.110');
